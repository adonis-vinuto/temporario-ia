...
export async function sendFirstMessage(
  ...
  console.log(`Enviando primeira mensagem para agente ${agentId} no módulo ${module}`);
  console.log(`URL: ${API_BASE_URL}/api/${module}/chat/${agentId}/first-message`);
  console.log(`Dados enviados: ${JSON.stringify({ message })}`);
  if (!response.ok) {
