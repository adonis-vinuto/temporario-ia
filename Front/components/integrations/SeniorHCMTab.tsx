...
import { SeniorHCMModal } from "./SeniorHCMModal";
import { SeniorHCMConfig, SeniorHCMConfigRequest } from "@/lib/interface/SeniorHCMConfig";
import {
  getSeniorHCMConfigs,
  createSeniorHCMConfig,
  updateSeniorHCMConfig,
  deleteSeniorHCMConfig,
} from "@/app/(app)/integrations/api/seniorHCMApi";
...
export function SeniorHCMTab() {
  const { currentModule } = useModule();
  const [configs, setConfigs] = useState<SeniorHCMConfig[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalMode, setModalMode] = useState<"create" | "edit" | "view">("create");
  const [selectedConfig, setSelectedConfig] = useState<SeniorHCMConfig | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [configToDelete, setConfigToDelete] = useState<SeniorHCMConfig | null>(null);
  ...
  const loadConfigs = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getSeniorHCMConfigs(currentModule);
      setConfigs(data);
    } catch (error) {
      console.error("Erro ao carregar configurações:", error);
      toast({
        title: "Erro",
        description: "Não foi possível carregar as configurações do Senior HCM",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleView = (config: SeniorHCMConfig) => {
  ...
  const handleEdit = (config: SeniorHCMConfig) => {
    setSelectedConfig(config);
    setModalMode("edit");
    setTimeout(() => {
      setModalOpen(true);
  ...
  const handleDeleteClick = (config: SeniorHCMConfig) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteSeniorHCMConfig(currentModule, configToDelete.idSeniorHcmConfig);
      toast({
        title: "Sucesso",
  ...
  const handleSave = async (data: Partial<SeniorHCMConfig>) => {
    try {
      const requestData: SeniorHCMConfigRequest = {
      ...
      if (modalMode === "create") {
        await createSeniorHCMConfig(currentModule, requestData);
        toast({
          title: "Sucesso",
          description: "Configuração criada com sucesso",
        });
      } else if (modalMode === "edit" && selectedConfig) {
        await updateSeniorHCMConfig(
          currentModule,
          selectedConfig.idSeniorHcmConfig,
          requestData
        );
        toast({
          title: "Sucesso",
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <p className="text-gray-600 dark:text-gray-400">
          Gerencie as configurações de integração com o Senior HCM
        </p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {configs.map((config) => (
            <CustomCard key={config.idSeniorHcmConfig}>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Senior HCM</p>
                    </div>
                  </div>
                  <DropdownMenu>
