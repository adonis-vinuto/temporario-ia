...
import { SeniorERPConfig } from "@/lib/interface/SeniorERPConfig";
import { Eye, EyeOff } from "lucide-react";

interface SeniorERPModalProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: Partial<SeniorERPConfig>) => Promise<void>;
  config?: SeniorERPConfig | null;
  mode: "create" | "edit" | "view";
}

export function SeniorERPModal({ 
  open, 
  onClose, 
  onSave, 
  config, 
  ...
  const [formData, setFormData] = useState<Partial<SeniorERPConfig>>({});
  const [loading, setLoading] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [isClosing, setIsClosing] = useState(false);

  // Key única para forçar remontagem completa
  const modalKey = `${mode}-${config?.idSeniorErpConfig || 'new'}-${open}`;
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle>
            {mode === "create" && "Nova Configuração Senior ERP"}
            {mode === "edit" && "Editar Configuração Senior ERP"}
            {mode === "view" && "Visualizar Configuração Senior ERP"}
          </DialogTitle>
        </DialogHeader>
